...
(*
   Given an amount n, one has to find a sum of numbers from the denominations list
   with the lowest number of summands.
*)
...
(*
	(Recursive calls will be n minus each of the denominations smaller than n.)
   Given an amount n, for each bill, let s_b denote the recursively computed shortest
   list to change n_bill. Take the shortest s_b and add its corresponding bill b
   to it to change the amount n.
*)
