...
module Cmp_String = struct
	type t = string
	let compare s t = if Pervasives.compare s t = -1
		then LT
		else if Pervasives.compare s t = 0
		then EQ
		else GT
end
...
module Cmp_inv (Cmp : Comparable) : Comparable with type t = Cmp.t  = struct
  type t = Cmp.t
  let compare x y = Cmp.compare y x
end
